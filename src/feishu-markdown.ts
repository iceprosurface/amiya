...
type FeishuAtElement = {
  tag: 'at'
  user_id: string
  user_name: string
}

type FeishuImageElement = {
  tag: 'img'
  image_key: string
}

type FeishuEmojiElement = {
  tag: 'emoji'
  emoji: string
}

type FeishuMdElement = {
  tag: 'md'
  text: string
}

type FeishuHrElement = {
  tag: 'hr'
}
...
function renderInlineChildren(children: unknown[]): InlineRenderResult {
  ...
  const renderInlineNode = (node: unknown) => {
    ...
    if (node.type === 'strong') {
      pushText('**')
      pushElements(
        renderInlineChildren(getChildren(node)).lines.flatMap((line, idx) => {
...
function renderCodeBlock(node: unknown): FeishuLine[] {
  ...
  return [[textElement(fenceStart)], ...lines.map((line) => [textElement(line)]), [textElement('```')]]
}
  ...
  try {
    ...
    if (first && hasType(first, 'heading') && getNumber(first, 'depth') === 1) {
      ...
      if (h1) {
        ...
        const position = isRecord(first)
          ? (first as Record<string, unknown>).position
          : undefined
    ...
    const content = clampPostLines(
      splitLines(trimmed).map((line) => [textElement(line)]),
    )
    return { content }
  }
}
